 ...
 * @throws FileNotFoundException *
  ...
  /************************************************************************
   * Method: ReadRules * Returns: void * Receives: * Purpose: read rules in from stemRules and
  ...
  /****************************************************************
   * Method: FirstVowel * Returns: int * Recievs: String word, int last * Purpose: checks lower-case
   * word for position of * the first vowel *
   ****************************************************************/
  ...
  /************************************************************************
   * Method: stripSuffixes * Returns: String * Recievs: String word * Purpose: strips suffix off
   * word and returns stem using * paice stemming algorithm *
   ************************************************************************/
  private String stripSuffixes(String word) {
    // integer variables 1 is positive, 0 undecided, -1 negative equiverlent of pun vars positive
    // undecided negative
    int ruleok = 0;
    int Continue = 0;
    // integer varables
    int pll = 0; // position of last letter
    int xl; // counter for nuber of chars to be replaced and length of stemmed word if rule was
            // aplied
    ...
    String stem = ""; // string holding the word as it is being stemmed this is returned as a
                      // stemmed word.
    // boolean varable
    boolean intact = true; // intact if the word has not yet been stemmed to determin a requirement
  ...
  /****************************************************************
   * Method: vowel * Returns: boolean * Recievs: char ch, char prev * Purpose: determin whether ch
   * is a vowel or not * uses prev determination when ch == y *
   ****************************************************************/
  ...
  /****************************************************************
   * Method: charCode * Returns: int * Recievs: char ch * Purpose: returns the relavent array index
   * for * specified char 'a' to 'z' *
   ****************************************************************/
  ...
  /********************************************************
   * Method: stripPrefixes * Returns: String * Recievs: String str * Purpose: removes prefixes so
   * that suffix * removal can comence *
   ********************************************************/
  ...
  /********************************************************
   * Method: Clean * Returns: String * Recievs: String str * Purpose: remove all non letter or digit
  ...
  /*************************** PUBLIC METHODS ****************/
  /****************************************************************
   * Method: stripAffixes * Returns: String * Recievs: String str * Purpose: prepares string and
   * calls stripPrefixes * and stripSuffixes *
   ****************************************************************/
  public String stripAffixes(String str) {
    // str = str.toLowerCase(); //change all letters in the input to lowercase
    // str = Clean(str); // remove all chars from string that are not a letter or a digit (why
    // digit?)
